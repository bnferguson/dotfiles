...
**Tools allowed**: `spawn_agent`, direct user questions, and `apply_patch` (for the research plan only)
...
**Tools allowed**: web search/page retrieval and `apply_patch`
...
**Tools allowed**: file search/read tools and `apply_patch`
...
The interview should be thorough but efficient. Ask the user directly in no more than 2-3 rounds.
...
Launch 10+ researcher agents with `spawn_agent`. Run them in batches that respect the current concurrency limit. Each agent receives a focused research assignment and a unique notes file.

**Spawning researcher agents:**

```
`spawn_agent` call with:
- `task_name`: a unique short subtopic slug
- `message`: Include these elements:
  1. Clear statement: "You are a RESEARCHER agent"
  2. Specific subtopic assignment
  3. Tool restrictions: "Only use web search/page retrieval and apply_patch"
...
YOUR TOOLS: Only use web search/page retrieval and apply_patch.

TASK:
1. Search the web for authoritative sources on quantum error correction implementation
2. Open and read promising sources
...
Launch as many researcher agents as the current concurrency limit permits. Wait for that batch before starting the remainder.
...
```
`spawn_agent` call with:
- `task_name`: `report_writer`
- `message`: Include these elements:
  1. Clear statement: "You are a REPORT-WRITER agent"
  2. Tool restrictions: "Only use file search/read tools and apply_patch"
...
YOUR TOOLS: Only use file search/read tools and apply_patch.

TASK:
1. List all files in research_notes/
2. Read each research notes file
...
- Fill the available concurrency slots, then launch another batch as agents finish
- Use `gpt-5.6-luna` for narrow researcher assignments when cost or latency matters
- Use `gpt-5.6-terra` with high reasoning for the report-writer when synthesis quality matters
