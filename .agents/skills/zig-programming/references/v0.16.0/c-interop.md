...
> Zig 0.16 deprecates `@cImport`. For new code, use `b.addTranslateC` in `build.zig`, expose its module through `imports`, and use `@import("c")` in Zig source. The sections below document the legacy builtin for migration work.
...
For WASI preopens, accept `std.process.Init` in `main` and use `init.preopens`. The parent process supplies this data with standard input, output, and error streams.
